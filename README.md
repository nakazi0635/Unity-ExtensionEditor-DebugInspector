...
すべてのゲームオブジェクトとそのスクリプトを一覧表示するクラス

各スクリプト内の変数とその値が表示され、表示される情報は、オブジェクト名、スクリプト名、変数名、値という順序で列挙される

スクロール機能や検索機能付き

出力できる型は114行から142行で定義されている

定義されていない型は\<unknown>と出力されるそのため、追加したい型があれば143行目以降に追加
## 内容
